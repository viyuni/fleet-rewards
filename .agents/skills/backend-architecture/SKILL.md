...
Use cases should import the corresponding input/output types from `@internal/shared`.
...
Repositories must not import Elysia, app env, route schemas, JWT/auth route code, or request input types from `@internal/shared`. Repositories must not open transactions, make permission decisions, hash passwords, sign tokens, or orchestrate business workflows.
...
- If code is shared schema or API contract, keep it in `shared/src/*`.
